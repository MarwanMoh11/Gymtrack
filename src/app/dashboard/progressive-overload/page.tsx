      ...
      if (dayLogString) {
        try {
          ...
          if (dayLog[exerciseId]) {
            ...
            if (Object.values(dayLog[exerciseId]).some(set => (set as LoggedSetData).isCompleted)) {
...
export default function ProgressiveOverloadDashboardPage() {
  ...
  useEffect(() => {
    ...
    // setIsLoading(false); // Loading state is now handled within fetchAISuggestion and the isClient check
  }, []); // Run once on mount
  ...
  const fetchAISuggestion = useCallback(async () => {
    if (!selectedExerciseId || !isClient) {
      ...
      setIsLoading(false); // Ensure loading is false if no suggestion needed
      return;
    }
  ...
  useEffect(() => {
    if (selectedExerciseId && isClient) {
        ...
        // Explicitly set loading false when no exercise is selected
        setIsLoading(false);
        setAiNextSessionSuggestion(null);
    }
  }, [selectedExerciseId, isClient, fetchAISuggestion]);
    ...
    <div className="container mx-auto py-8 px-4 md:px-6 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            ...
            />
            {/* Widget now handles the 'Select exercise' state internally */}
